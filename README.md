...
- **批量翻译**：支持拖拽导入图片、ZIP / CBZ / CBR / RAR 压缩包、PDF、EPUB、MOBI
...
从 [Releases](https://github.com/phdwdm-lang/bbq-translator/releases) 页面下载对应的扩展包文件：

| 扩展包 | 文件 | 大小 | 说明 |
|--------|------|------|------|
| **CUDA GPU 加速** | `cuda-gpu-extension.zip.001` + `.002` | 2.4 GB | NVIDIA 显卡加速推理，速度提升 3–10 倍。需驱动 ≥ 535，导入时 **同时选中 2 个文件**，完成后需重启 |
| **CTD 漫画文字检测器** | `ctd-detector-v1.0.zip` | 120 MB | 专为漫画优化的文字检测模型，气泡和文字区域检测更精准 |
| **LaMa Large 去字修复** | `lama_large_512px.zip` | 181 MB | 更强的图像修复模型，复杂背景去字效果更佳 |
| **MangaOCR 日文 OCR** | `manga-ocr-base.zip` | 393 MB | 日文识别效果最佳的 OCR 模型，针对日漫竖排文字优化 |
