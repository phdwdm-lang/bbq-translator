...
主程序默认使用 CPU 推理，如果你有 NVIDIA 独立显卡，可以安装 CUDA 扩展包以获得 **3–10 倍推理加速**。

**下载**：从 [Releases](https://github.com/phdwdm-lang/bbq-translator/releases) 页面找到 CUDA 扩展包，下载以下 **2 个文件**：
- `cuda-gpu-extension.zip.001`
- `cuda-gpu-extension.zip.002`

**导入步骤**：
1. 打开 BBQ翻译 → 设置（左侧边栏齿轮图标）→ 拓展
2. 找到 **CUDA GPU 加速** → 点击 **离线导入**
3. 在文件选择器中 **同时选中** 上述 2 个文件 → 确认
4. 等待导入完成（约 1–2 分钟），提示重启后点击 **立即重启**

> **注意**：CUDA 扩展包约 2.4 GB，需要 NVIDIA 显卡 + 驱动版本 ≥ 535。安装后可在拓展页面查看状态或卸载。
